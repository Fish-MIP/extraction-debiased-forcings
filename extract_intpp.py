...
for scenario in ['SSP245',  'SSP370',  'SSP585', 'SSP126']:
    ...
    filelist = glob(os.path.join(dirin, '*1m*diad_T*'))
    filelist.sort()
    filelist[:5]
    ...
    for f in filelist:
        ...
        int_ppdiat = (data['INTPPPHY2'])
        int_ppdiat.name = 'intppdiat'
        
        int_ppmisc = data['INTPPPHY']
        int_ppmisc.name = 'intppmisc'
        
        intpp = int_ppdiat + int_ppmisc
        intpp.name = 'intpp'
        ...
        for var in [int_ppdiat, int_ppmisc, intpp]: 
            # print(var)
